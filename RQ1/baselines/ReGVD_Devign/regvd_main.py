...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path, cwe_label_map):
        ...
        # 转换为模型所需格式
        for i in tqdm(range(len(funcs)), desc="Processing Data"):
...
def main():
    ...
    for task_id in range(5):
        ...
        train_data_file = args.train_data_file+str(task_id+1)+'/train.xlsx'
        eval_data_file = args.eval_data_file+str(task_id+1)+'/valid.xlsx'
        test_data_file = args.test_data_file+str(task_id+1)+'/test.xlsx'
        output_dir = args.output_dir
        model_name = args.model_name

        # Training phase
        logger.info(f"Loading training data for Task {task_id + 1}")
        train_dataset = TextDataset(tokenizer=tokenizer, args=args,
                                    cwe_label_map=cwe_label_map, file_path=train_data_file)
        eval_dataset = TextDataset(tokenizer=tokenizer, args=args,
        ...
        for eval_task_id in range(5):
            results = {}
            test_data_file = f"{args.test_data_file}{eval_task_id + 1}/test.xlsx"
