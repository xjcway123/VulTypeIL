...
class DataSet:
        ...
        with open(train_src, "r") as fp:
            ...
            for entry in tqdm(train_data):
                ...
                if entry[self.l_ident][0][0] < 30:
                    sequence = entry.get(self.s_ident, [])  # 如果没有找到 `sequence`，用空列表
                    example = DataEntry(datset=self, num_nodes=len(entry[self.n_ident]), features=entry[self.n_ident],
                                        edges=entry[self.g_ident], target=entry[self.l_ident][0][0], sequence=sequence)
                else:
                    sequence = entry.get(self.s_ident, [])
                    example = DataEntry(datset=self, num_nodes=len(entry[self.n_ident]), features=entry[self.n_ident],
                                        edges=entry[self.g_ident], target=30, sequence=sequence)
    ...
    def get_dataset_by_ids_for_GGNN(self, entries, ids):
        ...
        for entry in taken_entries:
            if not hasattr(entry, 'seq'):
                print(f"DataEntry missing 'seq': {entry}")
