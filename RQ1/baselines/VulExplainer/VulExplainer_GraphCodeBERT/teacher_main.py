...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, cwe_label_map, group_label_map, file_type="train"):
        ...
        group_labels = df["cwe_abstract_group"].tolist()
        one_hot_labels = []
        one_hot_group_labels = []
        for i in tqdm(range(len(labels))):
            # count label freq if it's training data
            if file_type == "train":
                cwe_label_map[labels[i]][2] += 1
            one_hot_labels.append(cwe_label_map[labels[i]][1])
            one_hot_group_labels.append(group_label_map[group_labels[i]])
        self.examples = convert_examples_to_features(funcs, one_hot_labels, one_hot_group_labels, tokenizer, args)
        if file_type == "train":
    ...
    def __getitem__(self, item):
        ...
        return (torch.tensor(self.examples[item].source_ids),
                ...
                torch.tensor(self.examples[item].group_labels).float())

def convert_examples_to_features(examples, labels, group_labels, tokenizer, args):
    features = []
...
def train(args, train_dataset, model, tokenizer, eval_dataset, cwe_label_map):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            source_ids, source_mask, position_idx, attn_mask, labels, group_labels = [x.to(args.device) for x in batch]
            model.train()
            loss = model(source_ids=source_ids, position_idx=position_idx, attn_mask=attn_mask, group=group_labels, labels=labels, return_prob=False)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        source_ids, source_mask, position_idx, att_mask, labels, group_labels = [x.to(args.device) for x in batch]
        with torch.no_grad():
            prob = model(source_ids=source_ids, position_idx=position_idx, attn_mask=att_mask, group=group_labels, labels=labels, return_prob=True)
...
def test(args, model, tokenizer, test_dataset):
    ...
    for batch in test_dataloader:
        source_ids, source_mask, position_idx, att_mask, labels, group_labels = [x.to(args.device) for x in batch]
        with torch.no_grad():
            prob = model(source_ids=source_ids, position_idx=position_idx, attn_mask=att_mask, labels=labels, group=group_labels, return_prob=True)
            y_preds += list((np.argmax(prob.cpu().numpy(), axis=1)))
            y_trues += list((np.argmax(labels.cpu().numpy(), axis=1)))
    # calculate scores
    acc = accuracy_score(y_trues, y_preds)
...
def main():
    ...
    model = CNNTeacherModel(shared_model=cnn_model, tokenizer=tokenizer, num_labels=len(cwe_label_map), args=args, hidden_size=300)
